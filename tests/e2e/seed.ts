 ...
 * Passages and server queries share deterministic local embeddings, so the
 * suite exercises KNN ranking without downloading the production model.
 */
async function seedDatabase() {
  if (process.env.CHAT_SEARCH_EMBEDDING_MODE !== "deterministic") {
    throw new Error("E2E seeding requires CHAT_SEARCH_EMBEDDING_MODE=deterministic");
  }
  ...
  const seed = async (
    ...
    const vectors = await embedPassages(texts.map(([, text]) => text));
    ...
    texts.forEach(([role, text], i) => {
      ...
      insertVec.run(BigInt(chunkId), Buffer.from(vectors[i].buffer));
    });
  };
...
seedDatabase()
  .then(() => console.log(`[e2e] seeded test database at ${E2E_DB_PATH}`))
  .catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
